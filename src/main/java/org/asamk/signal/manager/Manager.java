...
public class Manager implements Signal {
    ...
    public String uploadStickerPack(String path) throws IOException, StickerPackInvalidException {
        ...
        JsonStickerPack pack = parseStickerPack(zip);
        ...
        for (int i = 0; i < pack.stickers.size(); i++) {
            ...
            if (zip.getEntry(pack.stickers.get(i).file) == null) {
        ...
        if (pack.cover != null) {
            ...
            if (zip.getEntry(pack.cover.file) == null) {
        ...
        for (int i = 0; i < pack.stickers.size(); i++) {
            ...
            ZipEntry entry = zip.getEntry(pack.stickers.get(i).file);
            byte[] data = IOUtils.readFully(zip.getInputStream(entry));
        ...
        if (uniqueCover) {
            ...
            ZipEntry entry = zip.getEntry(pack.cover.file);
            byte[] data = IOUtils.readFully(zip.getInputStream(entry));
            messageSender.uploadSticker(new ByteArrayInputStream(data), data.length, packKey, attr);
        }
    ...
    private static JsonStickerPack parseStickerPack(ZipFile zip) throws IOException, StickerPackInvalidException {
        ...
        InputStream stream = zip.getInputStream(manifest);
        String json = org.whispersystems.signalservice.internal.util.Util.readFully(stream);
